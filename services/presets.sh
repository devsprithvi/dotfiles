...
preset_secret_specs() {
    case "$1" in
        vscode:tunnel)  : ;;  # tunnel auth is via interactive OAuth device flow, not a secret (see below)
    ...
    case "$spec" in
        vscode:tunnel)
            ...
            preset_authenticate() {
                ...
                # ── IMPORTANT: the tunnel service does NOT accept GitHub PATs. ──
                # `code tunnel user login --provider github --access-token <PAT>`
                # and `code tunnel user show` will BOTH report success, yet the
                # tunnel API (global.rel.tunnels.api.visualstudio.com) then answers
                # 401 Unauthorized — a documented, still-open limitation:
                #   https://github.com/microsoft/vscode/issues/310726
                # The tunnel credential must come from the interactive GitHub (or
                # Microsoft) OAuth *device* flow. That credential carries a refresh
                # token, is persisted via the file keychain above, and is reused on
                # every boot. So we NEVER log the tunnel in with GITHUB_PAT, and we
                # NEVER log out a working cached credential (doing so is what caused
                # the earlier restart loop).

                # ── Path A: a cached credential exists → reuse it (normal boot). ──
                if "$code" tunnel user show >/dev/null 2>&1; then
                    log_info "Using the existing cached tunnel credential."
                    return 0
                fi

                # ── Path B: no cached credential, and we have a terminal → device login. ──
                # This is interactive and one-time: the resulting credential persists
                # across reboots via the file keychain.
                if [[ -t 0 && -t 1 ]]; then
                    log_info "No cached tunnel credential — starting one-time interactive GitHub device login."
                    log_info "Follow the prompt at https://github.com/login/device. This is needed only once."
                    local out
                    if out="$("$code" tunnel user login --provider github 2>&1)"; then
                        log_success "Tunnel login complete — credential stored for future boots."
                        return 0
                    fi
                    log_error "Interactive tunnel login failed. VS Code CLI said:"
                    while IFS= read -r _l; do [[ -n "$_l" ]] && log_error "  ${_l}"; done <<<"${out}"
                    return 1
                fi

                # ── Path C: headless with no cached credential — cannot proceed. ──
                # There is no supported way to authenticate the tunnel non-interactively
                # (PATs are rejected by the service; see the issue linked above).
                log_error "No cached tunnel credential and no terminal for device login (headless)."
                log_error "Run this ONCE interactively on THIS machine to establish the credential:"
                log_error "  ${code} tunnel user login --provider github"
                log_error "Then restart this service. GitHub PATs are NOT accepted by the tunnel"
                log_error "service (microsoft/vscode#310726) — the device flow is required."
                return 1
            }
