...
# ── Secret Fetching ─────────────────────────────────────────────────────────
# Secret resolution lives in its own subsystem under secrets/: a provider-
# agnostic core (secrets/core.sh) with the VAR[=LOCATOR] grammar, the
# DOTFILES_SECRET_MAP indirection layer, and secret_hydrate; plus one file per
# backend under secrets/providers/ (infisical, env, …). The legacy
# fetch_infisical_secret helper is preserved in the infisical provider.
